...
void lexing()
{
    for ( int i = 0; i < totalLine; i++ )
    {
        ...
        {
            if ( oparetorCheck( eachLine[ i ].text[ j ] ) && oparetorCheckdup( eachLine[ i ].text[ j + 1 ] ) )  //oparetorChecking(done)
            {
                keyword_identifier_check( i, j );
                cout << "oparetor\n" << eachLine[ i ].text[ j ] << eachLine[ i ].text[ j + 1 ] << "\n" << eachLine[ i ].line << "\n";
                j += 2;
            }
            else if ( oparetorCheck( eachLine[ i ].text[ j ] ) )   //oparetorChecking(done)
            {
                keyword_identifier_check( i, j );
                cout << "oparetor\n" << eachLine[ i ].text[ j ] << "\n" << eachLine[ i ].line << "\n";
                j += 1;
            ...
            /*bad
            else if(otherCheck(eachLine[i].text[j])){
                cout << "other\n" << eachLine[i].text[j] << "\n" << eachLine[i].line << "\n" << j+1 << "\n";
                j += 1;
            }*/
            else if ( eachLine[ i ].text[ j ] == '\\' )   //charChecking(done)
            {
                keyword_identifier_check( i, j );
                cout << "character\n" << eachLine[ i ].text[ j ] << eachLine[ i ].text[ j + 1 ] << "\n" << eachLine[ i ].line << "\n";//check which character
                j += 2;
            }
            else if ( digitCheck(eachLine[ i ].text[ j ] ) )   //digitChecking(done)
            {
                keyword_identifier_check( i, j );
            ...
            else if ( eachLine[ i ].text[ j ] == '"' )   //stringChecking(done)
            {
                keyword_identifier_check( i, j );
                j++;
                int temp = j;
